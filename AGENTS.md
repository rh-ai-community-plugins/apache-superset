...
Currently, the codebase retains these hooks from the seed project: `useCurrentUser`, `useProjects`, `useFavoriteProjects`, `useK8sResources`, `useNamespaceSummary`, `useAccessReview`, and `useLastSelectedProject`. Some will be kept (e.g., `useAccessReview` for RBAC checks, `useLastSelectedProject` for project context), others replaced or removed as Superset-specific hooks are implemented.
